...
Также можно переиспользовать один и тот же объект Data, выделив память один раз, и каждый раз менять значение внутри объекта Data через сеттер, так как оно нигде не используется.
...
После использовани сеттера для любой кучи время выполнения стало 2.5 сек, кол-во Pause Young для 256мб было 2, для 512м -- 1,
для больших куч сборок мусора вообще не было. Далее в скриншотах графики для оптимизированной программы, но без использования сеттеров.
